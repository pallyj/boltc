use blirssa::typ::{Type, StructRef};
use inkwell::{types::{FloatType, IntType, FunctionType, BasicTypeEnum, BasicType, BasicMetadataTypeEnum, PointerType}, AddressSpace};
...
pub fn lower_basic_typ<'a, 'ctx>(t: &Type, context: &ModuleContext<'a, 'ctx>) -> Option<BasicTypeEnum<'ctx>> {
	Some(match t {
		...
		Type::Function { return_type, pars } => lower_function_type(&*return_type, pars, &context)?.ptr_type(AddressSpace::Global).as_basic_type_enum(),

		Type::Pointer { pointee } => lower_pointer_typ(&*pointee, context)?.as_basic_type_enum(),

		Type::Struct { container } => {
			lower_struct_typ(container, context)?.as_basic_type_enum()
		}
...
pub fn lower_struct_typ<'a, 'ctx>(container: &StructRef, context: &ModuleContext<'a, 'ctx>) -> Option<BasicTypeEnum<'ctx>> {
	if let Some(transparent) = container.transparent_type() {
		...
		Some(
			context.module
				.get_struct_type(container.name())?.as_basic_type_enum()
		)
	}
}
...
pub fn lower_pointer_typ<'a, 'ctx>(t: &Type, context: &ModuleContext<'a, 'ctx>) -> Option<PointerType<'ctx>> {
	Some(match t {
		...
		Type::Struct { container } => {
			lower_struct_typ(container, context)?
