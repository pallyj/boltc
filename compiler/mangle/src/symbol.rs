...
#[derive(Debug, Clone)]
pub struct Symbol {
	components: Vec<SymbolKind>,
}

impl Symbol {
	pub fn from_components(components: Vec<SymbolKind>) -> Symbol {
		Symbol { components }
	}

	pub fn new(component: SymbolKind) -> Symbol {
		Symbol { components: vec![component] }
	}

	pub fn append(mut self, component: SymbolKind) -> Self {
	...
	pub fn parse_list(from: &str) -> Option<Symbol> {
		...
		Some(Symbol::from_components(components))
	}
}