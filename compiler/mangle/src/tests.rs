use crate::{Path, defs::{MangledStruct, MangledFunction}, MangledType};
...
fn test_struct() {
	...
	assert_eq!("3std4File5ErrorS", MangledStruct(&file_error_path).to_string());
}
...
fn test_function() {
	...
	let args = vec![
		MangledType::StringSlice,
		MangledType::Integer1,
		file_type
	];

	let mangled = MangledFunction { path: &open_path, args, labels: vec![ Some("path"), None, None ]  };
