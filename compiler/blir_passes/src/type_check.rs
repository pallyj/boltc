use blir::{Library, typ::{Type, TypeKind, StructRef}, code::{FunctionRef, CodeBlock, Statement, StatementKind, MethodRef}, value::{Value, ValueKind, IfValue, IfBranch}};
...
fn check_if_value(if_value: &IfValue, mut spans: Vec<Span>, ty: &Type, debugger: &mut Debugger) {
	check_value(if_value.condition.as_ref(), &if_value.condition.typ, debugger);
