use blir::{typ::{StructRef, TypeKind}, value::ValueKind, code::{CodeBlock, StatementKind, Method, FuncParam}, attributes::Attributes, Visibility};
use errors::Span;

pub (crate) fn add_default_initializer(r#struct: &StructRef) {
	// Get the variables
	let struct_variables = r#struct
		.borrow()
		.instance_vars
		.clone();
	...
	init_statements.push(StatementKind::Bind {
		name: "self".to_string(),
		typ: self_type.clone(),
	...
	for variable in struct_variables {
		...
		let assigned_value =
		if let Some(default_value) = variable_default {
			default_value
		} else {
			let parameter_value = ValueKind::FunctionParam(variable_name.clone())
				.anon(variable_type.clone());
		...
		// Do an assignment
		let member = ValueKind::InstanceVariable {
			reciever: Box::new(self_value.clone()),
		...
		init_statements.push(StatementKind::Eval { value: assignment, escaped: true }.anon());
	}

	init_statements.push(StatementKind::Eval { value: self_value, escaped: false }.anon());

	let code = CodeBlock::new(init_statements, Span::empty());
	let func_params = parameter_types
		.into_iter()
		.zip(parameter_labels)
		.map(|(typ, label)| FuncParam { label: Some(label.clone()), bind_name: label, typ })
		.collect();
