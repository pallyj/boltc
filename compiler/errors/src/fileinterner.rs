...
impl FileInterner {
	...
	pub fn get_line_info<'a>(&'a self, file: usize, pos: usize) -> LineInfo<'a> {
		self.files[file].get_line_info(pos)
	}
...
impl File {
	pub fn new(file_name: String, text: String) -> File {
		...
		File { text, line_breaks, file_name }
	}
	...
	pub fn get_line_info<'a>(&'a self, n: usize) -> LineInfo {
		...
		LineInfo { line, col, text, filename }
	}
