...
impl OperatorPrecedence {
	pub fn shifts(&self, after: OperatorPrecedence) -> bool {
		return (*self as u32) > (after as u32)
	}
}
...
impl OperatorFactory {
	...
	pub fn register_infix(&mut self, name: &str, sym: &str, precedence: OperatorPrecedence) {
		self.register(Operator::new(name, sym, OperatorFix::Infix, precedence));
	}

	pub fn register_prefix(&mut self, name: &str, sym: &str) {
		self.register(Operator::new(name, sym, OperatorFix::Prefix, OperatorPrecedence::Prefix));
	}

	pub fn register_postfix(&mut self, name: &str, sym: &str) {
		self.register(Operator::new(name, sym, OperatorFix::Postfix, OperatorPrecedence::Postfix));
	}
	...
	pub fn parse_prefix_operator(&self, parser: &mut Parser) -> Option<&Operator> {
		let next = parser.peek().clone().unwrap().0;

		match next {
			Token::Punctuation(ref p) => {
				if let Some(op) = self.prefix_ops.get(p) {
					parser.consume();
					return Some(op)
				}
			}
	...
	pub fn parse_postfix_operator(&self, parser: &mut Parser, in_precedence: OperatorPrecedence) -> Option<&Operator> {
		let next = parser.peek().clone().unwrap().0;

		match next {
			Token::Punctuation(ref p) => {
				if let Some(op) = self.postfix_ops.get(p) {
					if op.precedence().shifts(in_precedence) {
						parser.consume();
						return Some(op)
					}
				}
