...
impl Struct {
	...
	pub fn fields(&self) -> Ref<Vec<StructField>> {
		self.fields.borrow()
	}
	...
	pub fn add_field(&self, field: StructField) {
		...
		self.field_indices.borrow_mut().insert(field.name.clone(), fields.len() as u32);
		fields.push(field);
	}
	...
	pub fn get_field_type(&self, name: &str) -> Type {
		let idx = *self.field_indices.borrow().get(name)
			.expect("Field doesn't exist") as usize;

		self.fields.borrow()[idx]
			.typ
			.clone()
	}

	pub fn get_field_index(&self, name: &str) -> u32 {
		*self.field_indices.borrow().get(name)
