...
impl Function {
	pub fn new(attributes: Attributes, visibility: Visibility, name: String, params: Vec<FuncParam>, return_type: Type, code: CodeBlock, span: Span, parent: &ScopeRef, parent_mangled: Mangled) -> FunctionRef {
...
impl FunctionRef {
	...
	pub fn take_name(&self) -> &str {
		unsafe { &*self.func.inner.as_ptr() }.info.name()
	}
}
...
impl Debug for FunctionRef {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(f, "{} func {}({}): {:?} {:?}", func.visibility, func.info.name, params, func.info.return_type, func.code)
    }
}
...
impl FunctionInfo {
	...
	pub fn always_links(&self) -> bool {
		self.always_link
	}

	pub fn always_link(&mut self) {
		self.always_link = true;
	}
