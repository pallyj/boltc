...
use crate::{typ::StructRef, BlirContext, code::FunctionInfo};
...
impl Attribute {
	pub fn new(attribute_name: String, span: Span) -> Self {
		Self {
			attribute_name,
			span
		}
...
impl Attributes {
	pub fn new(attributes: impl Iterator<Item = Attribute>) -> Self {
		Self {
...
impl AttributeFactory {
	...
	pub fn register_struct_attribute<T: StructAttribute + 'static> (
		&mut self,
		attribute: T
	) {
	...
	pub fn register_func_attribute<T: FuncAttribute + 'static> (
		&mut self,
		attribute: T
	) {
		let name = attribute.name().to_string();
	...
	pub fn apply_struct_attributes(
		&self,
		struct_ref: &StructRef,
		context: &mut BlirContext,
		debugger: &mut Debugger
	) {
		let attributes = struct_ref
			.borrow().attributes
			.clone();
	...
	pub fn apply_func_attributes(
		&self,
		attributes: &Attributes,
		func_info: &mut FunctionInfo,
		context: &mut BlirContext,
		debugger: &mut Debugger
	) {
		for attribute in attributes.iter() {
...
impl FuncAttribute for EntryPointAttribute {
    ...
    fn apply(&self, info: &mut FunctionInfo, context: &mut BlirContext, _debugger: &mut Debugger) {
